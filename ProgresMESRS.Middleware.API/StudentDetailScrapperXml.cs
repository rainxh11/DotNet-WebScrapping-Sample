...
namespace ProgresMESRS.Middleware.API
{
    ...
    public class StudentDetailScrapperXml : IPageScrapper<StudentDetail>
    {
        ...
        public StudentDetail ExtractSingleData(string pageHtml, string id)
        {
            ...
            result.Picture = DownloadStudentPicture(htmlDoc.DocumentNode);
            return result;
        }
        static byte[] DownloadStudentPicture(HtmlNode node)
        {
            try
            {
                var imageUrl = node
                ...
                if(result.Length == 0)
                {
                    Program.browser.Page.StartDownload($"https://progres.mesrs.dz{imageUrl}");
                }
